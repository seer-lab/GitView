...
SELECT c.commit_date, f.name FROM repositories AS r INNER JOIN commits AS c ON r.repo_id = c.repo_reference INNER JOIN file AS f ON c.commit_id = f.commit_reference WHERE r.repo_name LIKE 'titan' AND r.repo_owner LIKE 'thinkaurelius' ORDER BY c.commit_date
